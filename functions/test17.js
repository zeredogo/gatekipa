...
(async () => {
  try {
    ...
    console.log("Registering test cardholder on LIVE...");
    const regPayload = {
      first_name: "Test",
      last_name: "User",
      address: {
    ...
    try {
      ...
      console.error("Cardholder creation failed!", e.response?.data);
      return;
    }
    ...
    console.log("Success NGN:", JSON.stringify(res.data, null, 2));
  } catch (err) {
    if (err.response) {
