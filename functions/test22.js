const axios = require("axios");
const AES256 = require("aes-everywhere");

(async () => {
  try {
    const token = "at_live_";
    const secret = "sk_live_";

    const client = axios.create({
      baseURL: "https://issuecards.api.bridgecard.co/v1/issuing",
      headers: {
        "Content-Type": "application/json",
        "token": `Bearer ${token}`
      }
    });

    const crypto = require("crypto");
    const uniqueExt = crypto.randomBytes(4).toString("hex");

    console.log("Registering valid test cardholder on LIVE...");
    const regPayload = {
      first_name: "Test",
      last_name: "User",
      address: {
        address: "123 Test Street",
        city: "Lagos",
        state: "Lagos",
        country: "Nigeria",
        zip_code: "100001"
      },
      identity: {
        id_type: "NIGERIA_BANK_VERIFICATION_NUMBER",
        id_image: "https://example.com/id.jpg",
        bvn: "22222222222",
        id_no: "22222222222"
      },
      phone_code: "234",
      phone: "800000" + uniqueExt,
      email_address: `testuser${uniqueExt}@example.com`
    };

    let cardholder_id;
    try {
      const regRes = await client.post("/cardholder/register_cardholder_synchronously", regPayload);
      cardholder_id = regRes.data.data.cardholder_id;
      console.log("Cardholder created:", cardholder_id);
    } catch (e) {
      console.error("Cardholder creation failed!", JSON.stringify(e.response?.data, null, 2));
      return;
    }

    console.log("Creating card for", cardholder_id, "...");
    const encryptedPin = AES256.encrypt("1234", secret);

    const payload = {
      cardholder_id: cardholder_id,
      card_type: "virtual",
      card_brand: "Mastercard",
      card_currency: "NGN",
      pin: encryptedPin
    };
    
    const res = await client.post("/cards/create_card", payload);
    console.log("Success:", JSON.stringify(res.data, null, 2));
  } catch (err) {
    if (err.response) {
       console.error("Error Response Status:", err.response.status);
       console.error("Error Response Data:", JSON.stringify(err.response.data, null, 2));
    } else {
       console.error("Error:", err.message);
    }
  }
})();
