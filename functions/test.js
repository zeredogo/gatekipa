const admin = require("firebase-admin");
const axios = require("axios");
const AES256 = require("aes-everywhere");

admin.initializeApp({
  credential: admin.credential.applicationDefault(),
  projectId: "gatekipa-bbd1c"
});

const db = admin.firestore();

(async () => {
  try {
    const usersSnapshot = await db.collection("users").where("bridgecard_cardholder_id", "!=", null).limit(1).get();
    if (usersSnapshot.empty) {
      console.log("No user found with bridgecard_cardholder_id");
      return;
    }
    const userDoc = usersSnapshot.docs[0];
    const cardholder_id = userDoc.data().bridgecard_cardholder_id;
    console.log("Found cardholder_id:", cardholder_id);

    const token = "at_live_";
    const secret = "sk_live_";
    
    const client = axios.create({
      baseURL: "https://issuecards.api.bridgecard.co/v1/issuing",
      headers: {
        "Content-Type": "application/json",
        "token": `Bearer ${token}`
      }
    });

    const payload = {
      cardholder_id: cardholder_id,
      card_type: "virtual",
      card_brand: "Mastercard",
      card_currency: "NGN",
      pin: AES256.encrypt("1234", secret),
      meta_data: { test: "gatekipa" }
    };
    
    const res = await client.post("/cards/create_card", payload);
    console.log("Success NGN:", res.data);
  } catch (err) {
    console.error("Error NGN:", err.response?.data?.message || err.message);
  }
})();
