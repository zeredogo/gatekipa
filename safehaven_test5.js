...
async function run() {
  ...
  // 2. Validate Identity (Since the user says "ignoring the flow in the documentation", maybe Validate is required!)
  console.log('Validating verification with dummy OTP (will fail if OTP is wrong, but lets see)...');
  try {
    ...
    console.log('Validate Failed (expected due to bad OTP):', e.response?.data);
  }
  
  // 3. Create Sub-Account with exact payload (NO ibs_client_id, NO otp)
  const payload = {
  ...
  try {
    console.log('Attempting SubAccount with pure payload (Header ClientID = CLIENT_ID)...');
