...
async function run() {
  ...
  const payload = {
    validFor: 86400, // 1 day in seconds
    amountControl: "Fixed",
    amount: 1000,
  ...
  try {
    console.log('Attempting Virtual Account...');
    const res=await axios.post(BASE_URL+'/virtual-accounts',payload,{headers:{'Authorization':'Bearer '+token,'ClientID':CLIENT_ID,'Accept':'application/json','Content-Type':'application/json'}});
    console.log('SUCCESS:', res.data);
  } catch(e) {
    console.log('FAILED Virtual Account:', e.response?.status, JSON.stringify(e.response?.data, null, 2));
  }
}
