...
async function run() {
  ...
  // Try sub-account creation with correct header
  const payload = {
    ...
    identityId: '6a0618380f96d130c009ed68',
    autoSweep: false,
    firstName: "Test",
    lastName: "User"
  };
  ...
  try {
    console.log('Attempting SubAccount creation with ibs_client_id in ClientID header...');
    const res=await axios.post(BASE_URL+'/accounts/v2/subaccount',payload,{headers:{'Authorization':'Bearer '+token,'ClientID':ibs_client_id,'Accept':'application/json','Content-Type':'application/json'}});
